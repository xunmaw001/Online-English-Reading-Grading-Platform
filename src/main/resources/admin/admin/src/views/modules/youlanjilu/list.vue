        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '游览表名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.youlanTablename" placeholder="游览表名称" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.youlanName" placeholder="名称" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('youlanjilu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/zaixianyingyuyuedupingtai/upload/youlanjiluMuBan.xls"
                        >批量导入游览记录数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('youlanjilu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="youlanjiluUploadSuccess"
                                :on-error="youlanjiluUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('youlanjilu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入游览记录数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('youlanjilu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "youlanjilu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="游览表id">
                        <template slot-scope="scope">
                            {{scope.row.youlanTableid}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="youlanTablename"
                                   header-align="center"
                                   label="游览表名称">
                        <template slot-scope="scope">
                            {{scope.row.youlanTablename}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="名称">
                        <template slot-scope="scope">
                            {{scope.row.youlanName}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="youlanPhoto"
                                     header-align="center"
                                     width="200"
                                     label="图片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.youlanPhoto">
                                <img :src="$base.url+scope.row.youlanPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('youlanjilu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('youlanjilu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('youlanjilu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'youlanjilu',//当前表表名,
                        sumColum : 'youlanjilu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'youlanjilu_name',//分组字符串字段
                        // types : 'youlanjilu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.youlanTablename!= '' && this.searchForm.youlanTablename!= undefined) {
                    params['youlanTablename'] = '%' + this.searchForm.youlanTablename + '%'
                }
         
                if (this.searchForm.youlanName!= '' && this.searchForm.youlanName!= undefined) {
                    params['youlanName'] = '%' + this.searchForm.youlanName + '%'
                }
                        
                params['youlanjiluDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            youlanjiluUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "youlanjilu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "youlanjilu/update",
                        method: "post",
                        data: {
                            id:id,
//                            youlanjiluTypes:youlanjiluTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
